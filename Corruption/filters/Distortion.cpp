...
Distortion::Distortion(int dur)
    : m_dur(dur), m_start(std::chrono::high_resolution_clock::now()),
      m_end(std::chrono::milliseconds(0)), m_on(false) {}

void Distortion::setTime(int dur) { m_dur = dur; }
...
bool Distortion::isActive() {
  ...
  if (std::chrono::duration_cast<std::chrono::milliseconds>(
          (m_end + std::chrono::milliseconds(m_dur)) - m_start)
          .count() > 0)
    return true;
  return false;
}
